...
> 维护一个指向所有存在于物理内存中且可以置换的页表项的链表，当出现页缺失时，根据CLOCK算法遍历页表项，找到第一个访问位为0的页表项作为被替换的页，与FIFO不同的是不需要考虑新页表项插入的位置，直接放在之前旧页表项的位置即可，新页表项的访问位置为1。
> CLOCK相对LRU更易于实现，并且不需要过多硬件的支持。LRU需要在硬件层面支持链表的维护，按照X86设计思路，操作系统并不能知道应用程序访问内存的顺序，所以不能实现LRU。
