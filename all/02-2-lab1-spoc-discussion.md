...
描述符特权级DPL、当前特权级CPL和请求特权级RPL的含义是什么？在哪些寄存器中这些字段？对应的访问条件是什么？ (challenge)写出一些简单的小程序（c or asm）来体现这些特权级的区别和联系。
 ```
  ...
  - 
 ```
> CPL是当前特权级别；DPL是段访问的权限；RPL存储在段选择子中，是进程请求段访问的权限，其存在使得段访问更加安全，只有CPL和RPL是DPL要求的或者更高的权限的时候，进程才可以进行段访问，即便进程拥有很高的CPL，但是从其他进程那里获得的段选择子是地权限的，进程依然无法成功进行段访问，必须将段选择子RPL更新到更高的权限。
> CPL存在CS寄存器后两位，DPL存在段里，RPL存在DS/ES/SS/FS/GS寄存器的后两位。
> 访问条件 CPL <= DPL && RPL <= DPL
...
> 将用户态寄存器ss esp cs eip eflags的值压入到内核态堆栈中，从内核态恢复到用户态的时候从内核态堆栈中恢复这些寄存器。
